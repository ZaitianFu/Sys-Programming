...
vector *vector_create(copy_constructor_type copy_constructor,
	...
	newvec->capacity=get_new_capacity(INITIAL_CAPACITY);
	newvec->array=malloc(newvec->capacity*sizeof(void*));
	newvec->size=0;
...
void vector_insert(vector *this, size_t position, void *element) {
	...
	if (position>=this->size)
		position=this->size;
	vector_resize(this,this->size+1);
	void** newarray = malloc(sizeof(void*)* (this->capacity+1) );
	for(int i = 0; i <(int)position; i ++ ){
  			newarray[i]=this->array[i];
  			
  			}
	 newarray[(int)position]=element;
	for(int i =position; i < (int)this->size-1; i ++ ){
  			newarray[i+1] = this->array[i];
  		
  		}
	for(size_t i=0; i<this->size;i++)
		this->array[i]=this->copy_constructor(newarray[i]);
	free(newarray); newarray=NULL;
	//vector<void> vec; i
//(this->array, this->array + (sizeof(this->array)/ sizeof(this->array[0])));
...
void vector_erase(vector *this, size_t position) {
    ...
    // your code here
         ...
         void** newarray = malloc(sizeof(void*)* this->capacity );
         for(int i = 0; i <(int)position; i ++ ){
                         newarray[i]=this->array[i];
                         
                         }
         for(int i =position+1; i < (int)this->size; i ++ ){
                         newarray[i-1] = this->array[i];
