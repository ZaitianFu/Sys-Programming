...
void print_tlb(tlb *test_tlb) {
    if (DBG) {
        while (test_tlb) {
            printf("base_virtual_addr: [%u], value: [%p], next: [%p]\n",
                   test_tlb->base_virtual_addr, test_tlb->entry,
                   test_tlb->next);
...
void check_node(tlb *head, addr32 base_virtual_addr, page_table_entry *value,
                size_t index) {
    tlb *runner = head;
    while (index) {
        runner = runner->next;
...
int check_empty(tlb **head) {
    ...
    check_node(*head, (addr32)0, NULL, 0);
...
int insert_1_node(tlb **head) {
    ...
    addr32 base_virtual_addr = 0xDEADBEEF;
