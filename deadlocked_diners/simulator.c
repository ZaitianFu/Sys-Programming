...
static void *surveillance_start(void *cp) {
    ...
    while (running == 1) {
        ...
        if (working_threads == 0 && working == 0) {
            if (only_one_intern == 0) {
                deadlock = 1;
                break;
            }
        }
...
static size_t *arg_parse(int argc, char *argv[]) {
    if (argc != 3 && argc != 4) {
...
int main(int argc, char *argv[]) {
    ...
    // Wait for others!
    pthread_barrier_t company_barrier;
    pthread_barrier_init(&company_barrier, NULL, num_companies);
