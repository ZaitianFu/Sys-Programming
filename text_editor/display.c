...
/**
 *  Creates and initialized a new display for a given editor
 */
display *display_create(editor *editor, input_method handle_command) {
    display *new_display = malloc(sizeof(display));
    new_display->editor = editor;
    new_display->document = editor->document;
    new_display->filename = strdup(editor->filename);
    ...
    new_display->handle_command = handle_command;
...
void display_screen(display *d, int interact, const char *prompt,
    ...
    char *command = NULL;
    asprintf(&command, "p %zu %zu %zd %zd", d->top_line, d->start_col_idx,
             d->row - 1, d->col - TAB_WIDTH - 1);
    d->handle_command(d->editor, command);
    free(command);
    fflush(stdout);
    close(stdoutPipe[1]);
